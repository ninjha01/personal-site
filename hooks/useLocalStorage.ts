...
export function useLocalStorage<T>(args: { initialValue: T; key: string }): [T, (next: T) => void] {
    ...
    function initializeFromLocalStorage() {
      ...
      if (cachedValueString) {
        try {
          ...
          console.error(`Failed to parse value for key ${key} in localstorage. Value ${cachedValueString}`, err);
        }
      } else {
        return;
      }
  ...
  useEffect(
    function cacheInLocalStorage() {
      try {
        ...
        console.error(`Failed to stringify and set value for key ${key} in localstorage. Value ${internalValue}`, err);
      }
    },
    [key, internalValue]
  );
