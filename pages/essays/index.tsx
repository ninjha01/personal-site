...
export const Essays = () => {
  return (
    <>
      <Head>
        <title>Essays | Nishant Jha</title>
        <link rel="canonical" href={"https://nishantjha.org/essays"} />
      </Head>
      <Shell>
        <EssayContent />
      </Shell>
  ...
  return (
    <div className="mx-auto max-w-2xl lg:max-w-5xl">
      <header className="max-w-2xl">
        <h1 className="text-4xl font-bold tracking-tight text-zinc-100 sm:text-5xl">Essays</h1>
        ...
        <div className="md:border-l md:border-blue-300 md:pl-6">
          <div className="flex flex-col gap-16">
            {config.essays.map(essay => (
              <UIEssayCard key={essay.title} title={essay.title} description={essay.description} href={essay.url} />
            ))}
          </div>
        </div>
      </div>
...
const UIEssayCard = (props: { title: string; description: string; href: string }) => {
