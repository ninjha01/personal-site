  ...
  return (
    <div className="mx-auto max-w-2xl lg:max-w-5xl">
      <header className="max-w-2xl">
        <h1 className="text-4xl font-bold tracking-tight text-zinc-100 sm:text-5xl">UI LocalTools</h1>
        <div className="mt-6 text-base text-zinc-400">
          {"An exploration of User Interfaces for common scientific tools."}
        </div>
      </header>
        ...
        <div className="md:border-l md:border-blue-300 md:pl-6">
          <div className="flex flex-col gap-16">
            {config.localTools.map(tool => (
              <UILocalToolCard key={tool.title} title={tool.title} description={tool.description} href={tool.url} />
            ))}
          </div>
        </div>
      </div>
...
const UILocalToolCard = (props: { title: string; description: string; href: string }) => {
